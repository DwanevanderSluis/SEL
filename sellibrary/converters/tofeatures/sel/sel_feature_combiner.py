...
class SELLightFeatureCombiner:
    ...
    def __init__(self, light_feature_calculator, features_to_zero = []):
        # __ instance variables
        self.features_to_zero = features_to_zero
        self.light_feature_calculator = light_feature_calculator
    ...
    def calc_light_features(self, body, title, entity_list, entity_id_set, name_by_entity_id, title_entity_list,
        ...
        position_features_by_ent_id = self.light_feature_calculator.calc_positions(body, entity_list, entity_id_set)  # 1
        self.logger.info('calc_light_features 2')
        field_positions_by_ent_id = self.light_feature_calculator.calc_first_field_positions(body, title, entity_list, entity_id_set,
                                                                    title_entity_list)  # 2
        self.logger.info('calc_light_features 3')
        sentence_positions_by_ent_id = self.light_feature_calculator.calc_sentence_positions(body, entity_list, entity_id_set)  # 3
        self.logger.info('calc_light_features 4')
        frequency_by_ent_id = self.light_feature_calculator.calc_field_frequency(body, entity_list, title_entity_list)  # 4
        self.logger.info('calc_light_features 5')
        capitalization_by_ent_id = self.light_feature_calculator.calc_capitalization(body, entity_list, entity_id_set)  # 5
        self.logger.info('calc_light_features 6')
        uppercase_ratio_by_ent_id = self.light_feature_calculator.calc_uppercase_ratio(body, entity_list, entity_id_set)  # 6
        self.logger.info('calc_light_features 8.1')

        term_length_w_by_ent_id = self.light_feature_calculator.calc_average_term_length_in_words(body, entity_list, entity_id_set)  # 8.1
        self.logger.info('calc_light_features 8.2')
        term_length_c_by_ent_id = self.light_feature_calculator.calc_average_term_length_in_characters(entity_list, entity_id_set)  # 8.2
        self.logger.info('calc_light_features 11')

        title_by_ent_id = self.light_feature_calculator.calc_is_in_title(entity_list, title_entity_list)  # 11
        self.logger.info('calc_light_features 14')
        entity_frequency_by_ent_id = self.light_feature_calculator.calc_entity_frequency(body, entity_id_set, name_by_entity_id)  # 14
        ...
        if very_light:
            ...
            degrees_by_ent_id = self.light_feature_calculator.calc_degrees(entity_id_set)  # 20
        self.logger.info('calc_light_features 22')
        doc_length_by_ent_id = self.light_feature_calculator.calc_document_length(body, entity_id_set)  # 22
