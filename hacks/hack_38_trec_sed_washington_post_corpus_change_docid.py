...
class WashingtonPostSedder:
    ...
    def replace_docid_articles(self, filename):
        for docid in self.docid_set:

            new_id = self.docid_list.index(docid) + 99000000
            ...
            self.logger.info('%s %s %s %s %s', cmd, param1, param2, param3, param4)
...
if __name__ == "__main__":
    ...
    app.replace_docid_articles(FileLocations.get_dropbox_intermediate_path()+'wp/wp_minus_0.txt')
    app.replace_docid_articles(FileLocations.get_dropbox_intermediate_path() + 'wp_sentiment_simple.txt')
    app.replace_docid_articles(FileLocations.get_dropbox_intermediate_path() + 'wp_base_tf_simple_v2.txt')
